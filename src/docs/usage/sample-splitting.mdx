...
Datasets outside the comfortable world of academia often contain samples which contain multiple sentences. This is problematic in three ways:
...
VoiceSmith can automatically split samples in your dataset at sentence breaks, this works on all [supported languages](/).
...
4. Creating Splits:
   A sentence tokenizer is used to split the texts into sentences. The timestamps generated in step three
   will be used to split the audios according to the sentence breaks.
...
It is advised to at least skim through the table as sample splitting is not perfect. If there are samples which you don't want to be split,
you can select them using the checkboxes on the left and then click on "Do not split selected". Those samples will stay in the dataset but will not be split.

If you are happy with the list of splits to apply you can click on "Next".

### Creating Splits

VoiceSmith will now swap out the samples in your dataset with the splits in the previous table. After this is finished you can
